...
typedef enum SceAudioInErrorCode {
	...
	SCE_AUDIO_IN_ERROR_INVALID_PORT         = 0x80260101, //!< Defined but not explicitly emitted by FW 3.60 SceAudioin.
	...
	SCE_AUDIO_IN_ERROR_BUSY                 = 0x8026010A, //!< Defined but not explicitly emitted by FW 3.60 SceAudioin.
...
typedef enum SceAudioInPortType {
	...
	SCE_AUDIO_IN_PORT_TYPE_CAMERA = 0x20  //!< PhotoCam capture profile with transient route-change reporting.
} SceAudioInPortType;
 ...
 *
 * FW 3.60 has explicit backend-selection logic for values 0, 1, 2, 0x10, and
 * 0x20. The value -1 is used after initialization and suspend; selecting it
 * through the API does not disable the previously active backend.
 */
typedef enum SceAudioInInputMode {
	SCE_AUDIO_IN_INPUT_MODE_NONE        = -1,   //!< Internal no-current-route state.
